...
# Note: Bugs in the triton shift of INT data, so we shift the input data to UINT
# ********************************************************
import torch
import triton
...
def pack_weights_over_rows_torch(W_q: torch.Tensor, W_nbits: int, packing_bitwidth: int, transpose: bool)-> tuple[torch.Tensor, int]:
    ...
    for i in range(elements_per_sample):
        shift = i * W_nbits
        # 取所有列，第i组的所有行
        W_q_out += (W_q[i::elements_per_sample, :] << shift)
...
def test_pack_unpack_rows():
    # 创建一个简单的量化权重矩阵
    W_nbits = 4
    packing_bitwidth = 32
    rows, cols = 8, 16
    W_q = torch.randint(0, 2**W_nbits, (rows, cols), dtype=torch.uint8, device='cuda')

    # 打包
    packed, elements_per_sample = pack_weights_over_rows(W_q, W_nbits, packing_bitwidth, transpose=False)
    # packed, _ = pack_weights_over_rows_torch(W_q, W_nbits, packing_bitwidth, transpose=False)
    # 解包
    unpacked = unpack_over_rows(packed, W_nbits, W_q.shape[0], dtype=torch.uint8)

    # 检查是否一致
    assert torch.allclose(W_q, unpacked), "Pack/Unpack over rows failed!"
    print("Pack/Unpack over rows passed.")

...
def run(rank, world_size, tensor_size):
    ...
    os.environ['MASTER_PORT'] = '6585' # 可以是任意未被占用的端口
    ...
    # --- 2. 准备数据 ---
    device = f'cuda:{rank}'
    t_full = torch.rand(tensor_size, device=device, dtype=torch.float32)
    ...
    for i in range(world_size - 1):
        ...
        # 累加值
        t_chunks[recv_chunk_idx].add_(zero_buffer)
    ...
    # 比较自定义实现和 PyTorch 实现的结果
    # 注意：t_full 是通过 t_chunks 的视图重组的，它们共享内存
    if torch.allclose(t_full, t_full_pytorch, rtol=1e-4, atol=1e-4):
        print(f"Rank {rank}: ✅ Correctness check PASSED!")
    else:
        print(f"Rank {rank}: ❌ Correctness check FAILED!")
        # 打印一些调试信息
        diff = torch.sum(torch.abs(t_full - t_full_pytorch))
        print(f"   - Rank {rank} total absolute difference: {diff.item()}")
...
if __name__ == "__main__":
    ...
    if world_size < 2:
        ...
        # 使用mp.spawn启动多进程，它会自动为每个进程分配一个rank
        mp.spawn(run,

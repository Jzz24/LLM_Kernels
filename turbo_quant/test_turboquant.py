...
from turbo_quant.turboquant import (
    TurboQuantMSE,
    TurboQuantProd,
    TurboQuantKVCache,
    LloydMaxCodebook,
...
def test_lloyd_max_codebook():
    ...
    for d in [64, 128, 256]:
        for bits in [1, 2, 3, 4]:
            ...
            # 打印出具体的质心列表，保留4位小数以供观察
            formatted_centroids = [float(f"{c:.4f}") for c in cb.centroids.tolist()]
            print(f"    -> Centroids: {formatted_centroids}")
